...
/**
 * @Desc EnquiryNotes in enquiry module
 * @author Gunabalans
 */
include_once 'Util/CrudApi.php';

class EnquiryNotes extends Util\CrudApi {

    public function __construct($reg = NULL) {
        $this->crg = $reg;
        $this->crg->set('tn', 'notes'); //table name without prefix
...
//////////////////////////////////////////////////////////
    public function init() {
        ...
        $enqNotesID = filter_input(INPUT_POST, 'ID', FILTER_SANITIZE_STRING);
        
        switch ($this->HTTPMethodStringFromUrl) {
            case 'view':
                $this->checkMethod();
                
                $x = $this->db->select('notes', '*', ['ID' => filter_input(INPUT_GET, 'ID', FILTER_SANITIZE_STRING)]);               
                if($x){                   
            ...
            case 'delete':
                ...
                $x = $this->db->delete('notes', ['ID' => filter_input(INPUT_GET, 'ID', FILTER_SANITIZE_STRING)]);               
                if($x){                   
            ...
            case 'edit':
                ...
                $x = $this->db->update('notes', $ColInfo, $whereCondArray);
             ...
             case 'add':
                ...
                $x = $this->db->insert('notes', $ColInfo);
                //echo $this->db->last_query();
            ...
            case 'list':
                ...
                $getRow = $this->db->select('notes', '*', $whereCondArray);
