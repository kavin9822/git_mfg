...
{
// Insert a logo in the top-left corner at 200 dpi
// Image method signature:
// Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false)

$this->Image($imagePath,10,3,63,13,'PNG','','L',false,600,'',false,false,0,false,false,false);
}
...
// Client address
function addClientAdresse($to, $name, $adresse )
{
	$x1 = 10;
	$y1 = 42;
	//Positionnement en bas
...
function addReference($dear, $ref)
{
    $x1 = 10;
	$y1 = 85;
	//Positionnement en bas
	$this->SetXY( $x1, $y1 );
	$this->SetFont('Times','',10);
	$length = $this->GetStringWidth( $dear );
	$this->Cell( $length, 2, $dear);
	$this->SetXY( $x1, $y1 + 5 );
	$length = $this->GetStringWidth( $ref );
	//Coordonnées de la société
	$lignes = $this->sizeOfText( $ref, $length) ;
	$this->Cell($length, 2, $ref);
}
...
function addSignature()
{
	...
	$this->SetFont( "Times", "B", 8);
	$this->SetXY( $r1+15, $y1 );
	$this->Cell(15,4, "Authorised Signature", 0, 0, "");
	$this->SetFont( "Times", "B", 6);
	$this->SetXY( $r1 + ($r2-$r1)/2 - 5, $y1+18 );
	$this->Cell(10,2, "HARSHA VARDHAN SARAOGI", 0, 0, "C");
	$this->SetXY($r1 + ($r2-$r1)/2 - 5, $y1+20 );
	$this->Cell(10,2, "(Partner)", 0, 0, "C");
}

function addTermsCondt($terms)
{
	$this->SetFont( "Times", "B", 8);
	$r1  = 10;
	$r2  = $r1 + 120;
	...
	$this->Line( $r1, $y1+4, $r2, $y1+4);
	$this->SetXY( $r1+9, $y1);
	$this->Cell(30,4, "Terms and Conditions");
 	$this->SetFont( "Times", "", 7);
 	$this->SetXY( $r1+1, $y1+5 );
 	$this->MultiCell(118,3.5, $terms);

}

function addTermsCondtNew($terms,$sign)
{
	$x = $this->GetX();
    $y = $this->GetY();
	...
	$this->Cell(190,4, "TERMS AND CONDITIONS:-",0,0);
	$this->SetFont('Times','',10);
	$this->Ln();
	$x = $this->GetX();
    $y = $this->GetY();
	...
	$length = $this->GetStringWidth( $terms );
	$lignes = $this->sizeOfText( $terms, $length) ;
	$this->MultiCell(190, 4, $terms,0,'L',0,1,'','',true,0,false,true,0);
	$this->Ln();
	$x = $this->GetX();
    $y = $this->GetY();
	$this->SetXY( $x, $y );
	$length = $this->GetStringWidth( $sign );
	$lignes = $this->sizeOfText( $sign, $length) ;
	$this->MultiCell(190, 4, $sign,0,'L',0,1,'','',true,0,false,true,0);
...
function addDescription($desc)
{
	...
	$this->SetFont('Times','',10);
	$this->SetXY( $x, $y );
	$length = $this->GetStringWidth( $desc );
	$lignes = $this->sizeOfText( $desc, $length) ;
	
// MultiCell($w, $h, $txt, $border=0, $align='J', $fill=0, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0)

	$this->MultiCell(190, 4, $desc,0,'L',0,1,'','',true,0,false,true,0);
}

function addNoSignature($text){
    $x=$this->GetX();
    $y=$this->GetY();
    $this->SetXY( $x, $y );  
    $length = $this->GetStringWidth( $text );
	$lignes = $this->sizeOfText( $text, $length) ;
    $this->MultiCell(190, 4, $text,0,'L',0,1,'','',true,0,false,true,0);
}
