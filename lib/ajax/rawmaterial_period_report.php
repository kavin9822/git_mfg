...
if($_POST["PurchaseOrderID"]){
    ...
    $sql_query ="SELECT $supplier_table.ID,$supplier_table.SupplierName FROM $supplier_table 
                 LEFT JOIN $material_inward_table ON $material_inward_table.supplier_ID=$supplier_table.ID 
                 WHERE $material_inward_table.PurchaseNO=".$_POST['PurchaseOrderID']." GROUP BY $supplier_table.SupplierName";
