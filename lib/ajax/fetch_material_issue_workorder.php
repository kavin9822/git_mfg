  ...
  // $purchaseindent_table = $tablePrefix . 'purchaseindent';
    ...
    $entityID = $ajxSess->get('user')['entity_ID'];
      $sql_query ="SELECT 
                      ...
                      FROM 
                            $productionorder_table, 
                            $workorder_table
                            
                      WHERE 
                            $productionorder_table.ID = $workorder_table.PdnOrderID  
                      AND   $workorder_table.rmt_Status=1
                      AND   $workorder_table.statuss=1
                      AND   $productionorder_table.ID=".$_POST['ColumnValue']."";                
