...
if($_POST["ColumnValue"]){
  ...
  $supplier_table = $tablePrefix . 'supplier';
    ...
    $sql_query ="SELECT $productionorder_table.ID as pdnid,$supplier_table.ID as supplierid,$productionorder_table.PdnOrderNo,$supplier_table.SupplierName FROM $dispatchreturnable_table 
                 LEFT JOIN $productionorder_table ON $productionorder_table.ID=$dispatchreturnable_table.ProductionID  
                 LEFT JOIN $supplier_table ON $supplier_table.ID=$dispatchreturnable_table.SubcontractorID 
                 WHERE $dispatchreturnable_table.ID=".$_POST['ColumnValue']."";
