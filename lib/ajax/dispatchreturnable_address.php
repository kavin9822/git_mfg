...
if($_POST["ColumnValue"]){
  ...
  $supplier_table = $tablePrefix . 'supplier';
    ...
    $sql_query ="SELECT $productionorder_table.ID,$productionorder_table.PdnOrderNo,$supplier_table.AddressLine1 FROM $workorder_table  
                 LEFT JOIN $productionorder_table ON $workorder_table.PdnOrderID=$productionorder_table.ID 
                 LEFT JOIN $supplier_table ON $workorder_table.Subcontractor_ID=$supplier_table.ID 
                 WHERE $supplier_table.ID=".$_POST['ColumnValue']." GROUP BY $productionorder_table.ID";
