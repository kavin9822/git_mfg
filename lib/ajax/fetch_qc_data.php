...
if($_POST["ColumnValue"]){
  ...
  $purchaseindent_table = $tablePrefix . 'purchaseindent';
    ...
    $entityID = $ajxSess->get('user')['entity_ID'];
      ...
      $sql_query ="SELECT 
                            $material_inward_table.ID as MIN, 
                            $material_inward_table.MaterialNo, 
                            $material_inward_table.supplier_ID,
                      ...
                      FROM 
                            ...
                            $supplier_table 
                      WHERE 
                            $purchaseorder_table.ID = $material_inward_table.PurchaseNO   AND
                            $material_inward_table.supplier_ID = $supplier_table.ID   AND
                            $material_inward_table.mrn_Status = 1   AND
                            $purchaseorder_table.ID=".$_POST['ColumnValue']."";               
