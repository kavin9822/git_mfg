...
if($_POST["countryVal"]){
    ...
    $statecountrymap_tab = $tablePrefix . 'statecountrymap';
    $state_tab = $tablePrefix . 'state';
    $country_tab = $tablePrefix . 'country';
    //$entityID = $ajxSess->get('user')['entity_ID'];
  
    $sql_query  = "SELECT $state_tab.ID,$state_tab.StateName FROM $state_tab,$statecountrymap_tab WHERE $statecountrymap_tab.country_ID=".$_POST['countryVal']." AND $state_tab.ID=$statecountrymap_tab.state_ID ORDER BY $state_tab.StateName";
