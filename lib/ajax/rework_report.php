...
if($_POST["product_ID"]){
    ...
    $sql_query ="SELECT $supplier_table.ID,$supplier_table.SupplierName FROM $supplier_table 
                 LEFT JOIN $workorder_table ON $workorder_table.Subcontractor_ID=$supplier_table.ID 
                 WHERE $workorder_table.product_ID=".$_POST['product_ID']." GROUP BY $supplier_table.SupplierName";
