...
/////////////////////////////////////////
    ...
    protected function sgChangeMacId() {
	//name="ycs_ID"

        $yid = $_POST['ID'];
        $yEquipmentMac = trim($_POST['EquipmentMac']);
        ...
        if ( $rowData['SGloginID'] !== '' && $rowData['SGloginID'] !== NULL && $rowData['EquipmentMac'] !== $yEquipmentMac ) {
	  ...
	  $valid = filter_var($u_SGServerIP, FILTER_VALIDATE_IP);
        ...
        if ($u_SGServerIP !== '' && $u_SGServerIP !== NULL && $valid) {
            ...
            if ($responceData) {
               ...
               $yEquipmentMacQuote = $this->db->quote($yEquipmentMac);


                switch ($responcedataArr[0]) {
                    case 'Status:Success':
                        $sql = "UPDATE $table1 SET $table1.EquipmentMac = $yEquipmentMacQuote  WHERE $table1.ID = $yid";
