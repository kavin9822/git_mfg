        ...
        switch ($crud_string) {
            ...
            case 'edit':
                ...
                if ($rowData['SGuserRegStatus'] == 'YES') {
                    ...
                    //with message 
                    $this->tpl->set('message', 'This is not a SmartGAurd registered user');
...
/////////////////////////////////////////
    ...
    protected function sgReNewPackage() {


        $yid = $_POST['ID'];
        $ypackage_ID = $_POST['package_ID'];
        $renew_now = $_POST['renew_now'];
        ...
        if ($yid) {
            ...
            $yid = $this->db->quote($yid);
            $ypackage_ID = $this->db->quote($ypackage_ID);
            ...
            if ($responceData) {
                    ...
                    case 'Status:Success':
                        ...
                        $regdate = date("Y-m-d H:i:s");
                            ...
                            if($renew_now == "NO"){
                                ...
                                $expdt_sql = "SELECT PackExpireDate FROM $table1 WHERE ID = $yid";                                
                                $stmt = $this->db->prepare($expdt_sql);            
                                $result = $stmt->execute();
                                $expireDate  = $stmt->fetchAll();
                                $expire_date = $expireDate[0]['PackExpireDate'];
                                
                                
                               $sql = "UPDATE $table1 SET $table1.package_ID = $ypackage_ID ,"
                                    . "$table1.PackStartDate= '$expire_date',"
                                    . "$table1.PackExpireDate = DATE_ADD('$expire_date', INTERVAL $pacValid)"
                                    . "WHERE $table1.ID = $yid";                               
                        ...
                        try {
                            ...
                            $sqlcus= "SELECT $table1.FirstName,$table1.MobileNo,$table2.SMPackName FROM $table1,$table2 WHERE $table1.ID = $yid AND $table2.ID=$ypackage_ID";            
			    ...
			    $cus_nam_arr = preg_split("/[\s]+/",$cus_name);	
                        ...
                        if (!empty($CustomerMobileNo) && preg_match('/^\d{10}$/', $CustomerMobileNo)) {                           
                            ...
                            $smsMessage = 'Dear+' . $cust_fname . '!.+Ur+package+"' . $cust_pack . '"+is+renewed+on+' . date("j+M+Y+g:i+a");

                            include_once 'lib/util/curl.php';

                            $httpSms = new httpCurl();
                            if ($httpSms->smssigma($CustomerMobileNo, $smsMessage)) {
                             $this->tpl->set('message', $responcedataArr[1] . ' & SMS is sent to Customer Registered MobileNo.');   
                            }
                        } else {
                            ...
                            $this->tpl->set('message', $responcedataArr[1] . ' & SMS is not sent to customer!. Try Again');
                        }
