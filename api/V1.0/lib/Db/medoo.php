...
class medoo {
    ...
    public function __construct($options = null) {
        ...
        try {
            ...
            header(trim("HTTP/1.1 500 Internal Server Error"));
            $respomceBody = [
                'status' => 500,
                'message' => 'Internal Server Error',
                'description' => 'Pdo connection error (' . $e->getCode() . ') in datbase',
                'data' => '',
                'link' => ''
            ];
        ...
        if (is_array($where)) {
            ...
            if (isset($where['MATCH'])) {
                ...
                if (is_array($MATCH) && isset($MATCH['columns'], $MATCH['keyword'])) {
                    $where_clause .= ($where_clause != '' ? ' AND ' : ' WHERE ') . ' MATCH ("' . str_replace('.', '"."', implode($MATCH['columns'], '", "')) . '") AGAINST (' . $this->quote($MATCH['keyword']) . $mode .')';
                }
            }
    ...
    public function selectNum($table, $join, $columns = null, $where = null) {
        $column = $where == null ? $join : $columns;
...
//////var_dump($lastId);////
        ...
        return count($lastId) >= 1 ? $lastId[0] : FALSE;
    }
    ...
    public function have($table, $where = null) {
        $yx = $this->select($table, "*", $where);
