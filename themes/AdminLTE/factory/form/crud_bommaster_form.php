...
class Form_Elements {
    ...
    public static function data($reg, $form_data = NULL) {
        ...
        $mould_table = $crg->get('table_prefix') . 'mould';
        $mould_sql = "SELECT `ID`,`MouldName` FROM `$mould_table`";
        $mould_sel_data = $dbutil->getSqlData($mould_sql, 12);
        ...
        $FormConfig = array(
            ...
            'form_content' => array(
                ...
                'producttype_ID' => array(
                    ...
                    'status' => 'onchange = "Prodfilter(' . "'" . $crg->get('route')['base_path'] . "',this.value,'product_ID'" . ')" required',
                ),
                'product_ID' => array(
                    'filter' => TRUE,
                    'type' => 'select',
                ...
                'MouldQty' => array(
                    'filter' => TRUE,
                    'type' => 'text',
                    'value' => $form_data['MouldQty'],
                    'label' => 'Mould Quantity'
                ),
                'mould_ID' => array(
                    'filter' => TRUE,
                    'type' => 'select',
                    'value' => $form_data['mould_ID'],
                    'label' => 'Mould Name',
                    'options' => $mould_sel_data
                ),
                'entity_ID' => array(
                    'type' => 'hidden',
