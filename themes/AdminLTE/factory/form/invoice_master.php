...
class Form_Elements {
    ...
    public static function data($reg, $form_data = NULL) {
        ...
        if (!$ses->get('PaymentMode_option_data')) {
            
            $PaymentMode_option_data = array(
                '1'=>'Cash',
                '2'=>'Cheque'
            );
            
            $ses->set('PaymentMode_option_data', $PaymentMode_option_data);
        } else {
            $PaymentMode_option_data = $ses->get('PaymentMode_option_data');
        }
        
                

        //var_dump($ses->get('customer_type_option_data'));
        if (!$ses->get('customer_ID_option_data')) {
            $cust_type_selectBoxData = $dbutil->selectKeyVal('customer','ID','Name');
            $ses->set('customer_ID_option_data', $cust_type_selectBoxData);
        } else {
            $customer_ID_option_data = $ses->get('customer_ID_option_data');
        }
        ...
        $FormConfig = array(
             ...
             */
            'form_title' => 'Package Invoice Form',
            /*
