...
class Form_Elements {
    ...
    public static function data($reg, $form_data = NULL) {
        ...
        $dbutil = new DBUTIL($crg);
         ...
         $user_table = $crg->get('table_prefix') . 'users';
        $user_sql = "SELECT `ID`,`user_nicename` FROM `$user_table`";
        $user_sel_data = $dbutil->getSqlData($user_sql, 12);
        ...
        $FormConfig = array(
            ...
            'form_content' => array(
                ...
                'ProcessTypeName' => array(
                    'filter' => TRUE,
                    'type' => 'select',
                    'value' => $form_data['ProcessTypeName'],
                    'label' => 'Approval Type',
                    'options'=>$type_pdt_sel,
                    'status'=>'required'
                ),
                'approver_ID' => array(
                    'filter' => TRUE,
                    'type' => 'select',
                    'value' => $form_data['approver_ID'],
                    'label' => 'User',
                    'options'=>$user_sel_data,
