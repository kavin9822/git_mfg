...
class Form_Elements {
	...
	$customer_table = $crg->get('table_prefix') . 'customer';
        ...
        $buyer_table = $crg->get('table_prefix') . 'buyer';
        $byrsql = "SELECT `ID`,`Name` FROM $buyer_table WHERE `entity_ID`=$entityID";           
        $Buyer_ID_Sel = $dbutil->getSqlData($byrsql,12); 
        ...
        $FormConfig = array(
             ...
             */
            'form_title' => 'Product Invoice Form',
            /*
		...
		'users_ID' => array(
                ...
                'buyer_ID' => array(
                    'filter' => TRUE,
                    'type' => 'select',
                    'value' => $form_data['buyer_ID'],
                    'label' => 'Buyer ID',
                    'options' => $Buyer_ID_Sel
                ),
                'InvoiceType' => array(
                    'filter' => TRUE,
                    'type' => 'text',
