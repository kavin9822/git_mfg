...
class Form_Elements {
    ...
    public static function data($reg, $form_data = NULL) {
        ...
        $usertype_table = $crg->get('table_prefix') . 'usertype';
        $utsql = "SELECT `ID`,`Title` FROM `$usertype_table`";
        $user_type_sel = $dbutil->getSqlData($utsql, 12); 
        
        $entity_table = $crg->get('table_prefix') . 'entity';
        $entsql = "SELECT `ID`,`Title` FROM `$entity_table`";
        $entity_sel = $dbutil->getSqlData($entsql, 12);    
        ...
        $FormConfig = array(
             ...
             */
            'form_title' => 'User Activation Form',
            /*
            ...
            'form_content' => array(
                ...
                'user_mobileno' => array(
                    'filter' => TRUE,
                    'type' => 'number',
                    'value' => $form_data['user_mobileno'],
                    'label' => 'User MobileNo.',
                ),
                'usertype_ID' => array(
                    'filter' => TRUE,
                    'type' => 'select',
                    'value' => $form_data['usertype_ID'],
                    'label' => 'User Type',
                    'options' => $user_type_sel,
                     'status'=>'required'
                ),
                'entity_ID' => array(
                    'filter' => TRUE,
                    'type' => 'select',
                    'value' => $form_data['entity_ID'],
