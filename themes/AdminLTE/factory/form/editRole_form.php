...
class Form_Elements {
    ...
    public static function data($reg, $form_data = NULL) {
        ...
        $parentID = $crg->get('rbac')->Roles->parentNode($ycsID);
        
        
        //public mixed Rbac->{Entity}->getDescription(int $ID)
        $dec = $crg->get('rbac')->Roles->getDescription($ycsID);
        $tit = $crg->get('rbac')->Roles->getTitle($ycsID);
        ...
        //get parent if from value exanple
        //$gtParentID['No Parent ID']
        $getParentID = array_flip($ParentID_option_data);
        
        ///var_dump($getParentID);
        ...
        $FormConfig = array(
            ...
            'form_content' => array(
                ...
                'no_Parent_ID' => array(
                    'type' => 'hidden',
                    'value' => $getParentID['No Parent ID']
                ...
                'Parent_ID' => array(
                    'filter' => TRUE,
                    'type' => 'select',
                    'value' => $parentID['ID'] ? $parentID['ID'] : $getParentID['No Parent ID'],
                    'label' => 'Parent Role ID(Changing this will del and add role again)',
