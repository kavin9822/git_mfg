...
class Form_Elements {
    ...
    public static function data($reg, $form_data = NULL) {
        ...
        $brkdownreason_table = $crg->get('table_prefix') . 'breakdownreasons';
        $brkdownreason_sql= "SELECT `ID`,`Description` FROM `$brkdownreason_table`";
        $brkdownreason_sel_data= $dbutil->getSqlData($brkdownreason_sql,12);
        ...
        $FormConfig = array(
            ...
            'table_name' =>'productionbreakdowndet',
            /*
             ...
             */
            'form_title' => 'Production Breakdown Detail',
            /*
            ...
            'form_content' => array(
                'ID' => array(
                    ...
                    'type' => 'hidden',
                    'value' => $form_data['ID'],
                    'label' => 'Production Breakdown Detail ID'
                ),
                'pipeproduction_ID' => array(
                    'filter' => TRUE,
                    'type' => 'text',
                    'value' => $form_data['pipeproduction_ID'],
                    'label' => 'Pipe Production',
                    'status'=>'required'
                ),
                'machine_ID' => array(
                    'filter' => TRUE,
                ...
                'BreakDownStartTime' => array(
                    'filter' => TRUE,
                    'type' => 'text',
                    'value' => $form_data['BreakDownStartTime'],
                    'label' => 'BreakDown StartTime',
                    'status' =>'data-provide="datetimepicker" data-date-format="hh:mm" onclick="ycstime(this.id)"required',
                ),
                'BreakDownEndTime' => array(
                    'filter' => TRUE,
                    'type' => 'text',
                    'value' => $form_data['BreakDownEndTime'],
                    'label' => 'BreakDown EndTime',
                    'status' =>'data-provide="datetimepicker" data-date-format="hh:mm" onclick="ycstime(this.id)"required',
                ),
                //note spell mistake here
                'breaskdownreason_ID' => array(
                    'filter' => TRUE,
                    'type' =>'select',
                    'value' => $form_data['breaskdownreason_ID'],
                    'label' =>'Breakdown Reason',
                    'options'=>$brkdownreason_sel_data,
                    'status'=>'required'
                ),
                 'Remarks' => array(
                    'filter' => TRUE,
